...
function EditProfilePageContent() {
  ...
  const [originalProfileWidget, setOriginalProfileWidget] =
    useState('statistics');
  ...
  const buildUrl = (platform, value) => {
    ...
    switch (platform) {
      ...
      case 'youtube':
        return v.startsWith('@')
          ? `https://youtube.com/${v}`
          : `https://youtube.com/${v.startsWith('channel/') || v.startsWith('c/') ? v : `@${v}`}`;
      case 'telegram':
        return `https://t.me/${v}`;
      case 'discord':
        return /discord\.(gg|com)\//i.test(v)
          ? `https://${v}`.replace(/^https:\/\//, 'https://')
          : `https://discord.gg/${v}`;
      case 'vk':
  ...
  const handleInputBlur = (platform) => {
    setSocialLinks((prev) => ({
      ...prev,
      [platform]: autoPrefix(platform, prev[platform]),
    }));
  };
  ...
  const handleSave = async (e) => {
    ...
    if (!user) {
      setError(
        t('edit_profile.userError', {
          defaultValue: 'Пользователь не найден. Войдите снова.',
        })
      );
      return;
    }
    ...
    try {
      ...
      if (!session.data.session) {
        setError(
          t('edit_profile.sessionExpired', {
            defaultValue: 'Сессия истекла. Войдите заново.',
          })
        );
        return;
      }
      ...
      if (birthday && isNaN(new Date(birthday).getTime())) {
        setError(
          t('edit_profile.invalidBirthday', {
            defaultValue: 'Неверный формат даты. Используйте ГГГГ-ММ-ДД.',
          })
        );
        return;
      }
      ...
      for (const [platform, url] of Object.entries(socialLinks)) {
        ...
        if (cleaned && !/^https?:\/\//i.test(cleaned)) {
          setError(
            t('edit_profile.invalidUrl', {
              defaultValue: `Некорректная ссылка для ${platform}. Добавьте http:// или https://`,
            })
          );
          setSaving(false);
          return;
      ...
      if (
        ...
        JSON.stringify(originalSocialLinks)
      )
        updates.social_links = normalizedSocialLinks;
      if (profileWidget !== originalProfileWidget)
        updates.profile_widget = profileWidget;
      ...
      if (Object.keys(updates).length === 0) {
        setError(
          t('edit_profile.noChanges', {
            defaultValue: 'Нет изменений для сохранения.',
          })
        );
        setSaving(false);
        return;
      ...
      setSuccessMessage({
        key: 'edit_profile.successMessage',
        defaultValue: 'Профиль обновлен.',
      });
      ...
      setError({
        key: 'edit_profile.saveError',
        options: {
          message: err.message,
          defaultValue: 'Не удалось сохранить профиль: {{message}}',
        },
      });
    } finally {
      setSaving(false);
  ...
  return (
    <div className={styles.container}>
      ...
      <h1 className={styles.title}>
        {t('profile.edit.title', { defaultValue: 'Редактирование профиля' })}
      </h1>
      <form onSubmit={handleSave} className={styles.form}>
        <div className={styles.formGroup}>
          <label className={styles.label} htmlFor="birthday">
            {t('profile.edit.birthday', { defaultValue: 'Дата рождения' })}
          </label>
          <input
            id="birthday"
            type="date"
            ...
            aria-label={t('profile.edit.birthday')}
          />
        </div>
        ...
        <div className={styles.formGroup}>
          <label className={styles.label} htmlFor="description">
            {t('profile.edit.description', { defaultValue: 'Описание' })}
          </label>
          <textarea
            id="description"
            value={description}
        ...
        <div className={styles.formGroup}>
          <label className={styles.label}>
            {t('profile.edit.widgetLabel', {
              defaultValue: 'Виджет профиля на странице',
            })}
          </label>
          <button
            type="button"
          ...
          >
            {profileWidget === 'statistics'
              ? t('profile.edit.widgetStatistics', {
                  defaultValue: 'Статистика',
                })
              : t('profile.edit.widgetAchievements', {
                  defaultValue: 'Достижения',
                })}
          </button>
        </div>
        ...
        {showModal && (
          ...
          >
            ...
            >
              ...
              <div className={styles.widgetPreview}>
                ...
                >
                  ...
                  <div className={styles.previewBox}>
                    <p>
                      {t('profile.edit.subscribers', {
                        defaultValue: 'Подписчики',
                      })}
                      : 1000
                    </p>
                    <p>
                      {t('profile.edit.views', { defaultValue: 'Просмотры' })}:
                      50000
                    </p>
                ...
                >
                  ...
                  <div className={styles.previewBox}>
                    <p>
                      {t('profile.edit.rareAchievement1', {
                        defaultValue: 'Редкое достижение #1',
                      })}{' '}
                      (0.5%{' '}
                      {t('profile.edit.players', { defaultValue: 'игроков' })})
                    </p>
                    <p>
                      {t('profile.edit.rareAchievement2', {
                        defaultValue: 'Редкое достижение #2',
                      })}{' '}
                      (1%{' '}
                      {t('profile.edit.players', { defaultValue: 'игроков' })})
                    </p>
              ...
              <button
                onClick={() => {
                  setProfileWidget(tempWidget);
                  setShowModal(false);
                }}
              >
                {t('profile.edit.close', { defaultValue: 'Готово' })}
              </button>
            </div>
          </div>
        )}
        ...
        <fieldset className={styles.fieldset}>
          <legend className={styles.legend}>
            {t('profile.edit.socialLinks', { defaultValue: 'Соцсети' })}
          </legend>
          <div className={styles.socialLinksContainer}>
            {[
              ...
              <div key={platform} className={styles.socialLinkItem}>
                <label className={styles.socialLinkLabel} htmlFor={platform}>
                ...
                <input
                  ...
                  className={styles.input}
                  placeholder={t(
                    `edit_profile.socialsPlaceholder.${platform}`,
                    { defaultValue: 'вставьте ссылку или ник' }
                  )}
                />
                <div className={styles.hint}>
                  {t('edit_profile.socialsHint', {
                    defaultValue:
                      'Можно вставить ссылку полностью или ник — мы подставим префикс автоматически.',
                  })}
                </div>
              </div>
            ))}
        ...
        {error && (
          <p className={styles.errorMessage}>
            {typeof error === 'string' ? error : t(error.key, error.options)}
          </p>
        )}
        {successMessage && (
          <p className={styles.successMessage}>
            {t(successMessage.key, {
        ...
        <button type="submit" className={styles.saveButton} disabled={saving}>
          {saving
            ? t('profile.edit.saving', { defaultValue: 'Сохраняем…' })
            : t('profile.save', { defaultValue: 'Сохранить' })}
        </button>
      </form>
    </div>
  );
