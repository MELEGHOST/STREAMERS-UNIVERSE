...
function ProfilePageContent() {
  ...
  const { data, error: swrError } = useSWR(
    twitchUserId && authToken !== null
      ? [`/api/twitch/user?userId=${twitchUserId}`, authToken]
      : null,
  ...
  const fetchAllData = useCallback(async () => {
    ...
    console.log(
      `[ProfilePage] Загрузка ВСЕХ данных для twitchUserId: ${twitchUserId}, userId: ${user.id}`
    );
    setLoading(true);
    setError(null);
  ...
  if (swrError || error) {
    return (
      <div className={pageStyles.container}>
        <p className={pageStyles.errorMessage}>{swrError?.message || error}</p>
      </div>
    );
  }
  ...
  const StatsWidget = () => {
    const stats = [
      {
        title: t('followers'),
        value: twitchUserData?.followers_count || 0,
        icon: '👥',
      },
      // TODO: Заменить на реальные данные о просмотрах, если Twitch API их отдает
      {
        title: t('profile_page.views'),
        value: twitchUserData?.view_count || 0,
        icon: '👁️',
      },
      // TODO: Придумать еще статы. Например, дата регистрации на Twitch
      {
        title: t('profile_page.registrationDate'),
        value: twitchUserData
          ? new Date(twitchUserData.created_at).toLocaleDateString()
          : 'N/A',
        icon: '📅',
      },
    ];
  ...
  const AchievementsWidget = () => {
    const { data: apiPayload, error: achError } = useSWR(
      authToken !== null ? ['/api/achievements', authToken] : null,
      ([url, token]) => fetcher(url, token)
    );
    if (achError) {
      return (
        <div className={pageStyles.errorMessage}>
          {t('profile_page.achievements_page.error', {
            message: achError.message || 'Failed to load',
          })}
        </div>
      );
    }
    const achievements = Array.isArray(apiPayload)
      ? apiPayload
      : Array.isArray(apiPayload?.achievements)
        ? apiPayload.achievements
        : [];
        ...
        {unlockedAchievements.length > 0 ? (
          <ul>
            {unlockedAchievements.map((ach) => (
              <li key={ach.id}>
                {ach.name} ({Number(ach.rarity || 0).toFixed(1)}%{' '}
  ...
  const ReviewsWidget = ({ twitchUserId }) => {
      ...
      <div className={styles.widget}>
        ...
        {reviewsError ? (
          ...
          <ul>
            {reviews.map((review) => (
              <li key={review.id}>
                <strong>{review.author ?? t('common.unknown')}:</strong> &quot;
                {review.text}&quot; ({'★'.repeat(Number(review.rating) || 0)})
              </li>
            ))}
          </ul>
        ) : (
        ...
        <Link
          href={`/reviews/streamer/${twitchUserId}`}
  ...
  return (
    <I18nProvider>
      <div className={pageStyles.container}>
        ...
        >
          <button onClick={() => router.back()} className={styles.backButton}>
            {t('profile.back', { defaultValue: 'Назад' })}
          </button>
        </div>
        <header
        ...
        <div className={styles.actionsBar}>
          ...
          >
            <FaEdit />{' '}
            {t('profile_page.editProfile', {
              defaultValue: 'Редактировать профиль',
            })}
          </button>
          <button
          ...
          >
            <FaPlus />{' '}
            {t('profile_page.addReview', { defaultValue: 'Добавить отзыв' })}
          </button>
          <button
          ...
          >
            <FaTrophy />{' '}
            {t('profile_page.myAchievements', {
              defaultValue: 'Мои достижения',
            })}
          </button>
          {userRole === 'admin' && (
            <button
            ...
            >
              <FaShieldAlt />{' '}
              {t('profile_page.adminPanel', { defaultValue: 'Админ-панель' })}
            </button>
          )}
        ...
        <main className={styles.mainContent}>
          <div className={styles.widgetsGrid}>
            <StatsWidget />
            <AchievementsWidget />
            <ReviewsWidget twitchUserId={twitchUserId} />
            {/* Другие виджеты можно будет добавлять сюда */}
          </div>
          <section className={styles.profileSection}>
            <h2 className={styles.sectionTitle}>{t('profile.videos')}</h2>
            <div className={styles.videosGrid}>
              {Array.isArray(twitchUserData?.videos) &&
              twitchUserData.videos.length > 0 ? (
                twitchUserData.videos.map((video) => (
                  <div key={video.id} className={styles.videoItem}>
                    <Link
                      href={`https://www.twitch.tv/${video.user_login}/video/${video.id}`}
                      target="_blank"
                        ...
                        src={
                          ...
                          typeof video.thumbnail_url === 'string'
                            ? video.thumbnail_url
                                .replace('%{width}', '200')
                                .replace('%{height}', '112')
                            : 'https://static-cdn.jtvnw.net/jtv_user_pictures/placeholder-user.png'
                        }
                        alt={video.title}
                        width={200}
                        height={112}
                ...
                <p>
                  {t('profile.noVideos', { defaultValue: 'Видео недоступны' })}
                </p>
              )}
            </div>
          </section>
      ...
      {isOverlayOpen && (
        <AvatarSocialOverlay
          avatarUrl={
            twitchUserData?.profile_image_url || user.user_metadata.avatar_url
          }
          displayName={displayName}
