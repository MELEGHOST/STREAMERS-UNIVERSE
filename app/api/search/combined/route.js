...
import {
  ...
  getTwitchUsers,
} from '../../../utils/twitchClient.js'; // Убедимся, что путь верный
...
export async function GET(request) {
  ...
  try {
    ...
    try {
      console.log(
        '[API /search/combined] Searching in Supabase (user_profiles)...'
      );
      const { data: profileData, error: profileError } = await supabase
        .from('user_profiles')
        .select(
          'user_id, twitch_user_id, twitch_display_name, twitch_profile_image_url'
        )
        .ilike('twitch_display_name', `%${query}%`)
        .limit(10);
      if (profileError) {
        console.error(
          '[API /search/combined] Supabase search error:',
          profileError
        );
      } else {
        supabaseUsersData = profileData || [];
        console.log(
          `[API /search/combined] Found ${supabaseUsersData.length} potential profiles in Supabase.`
        );
      }
    } catch (dbError) {
      console.error(
        '[API /search/combined] Critical Supabase search error:',
        dbError
      );
    }

    // Получаем логины из auth.users (только если доступен сервисный ключ)
    let supabaseUsersWithLogin = supabaseUsersData.map((p) => ({
      ...p,
      login: null,
    }));
    if (supabaseAdmin && supabaseUsersData.length > 0) {
      ...
      try {
        console.log(
          `[API /search/combined] Fetching auth users to enrich login for ${userIds.length} users...`
        );
        const {
          data: { users: authUsersData },
          error: authUsersError,
        } = await supabaseAdmin.auth.admin.listUsers();
        if (authUsersError) {
          console.error(
            '[API /search/combined] Error fetching from auth.users:',
            authUsersError
          );
        } else {
          supabaseUsersWithLogin = supabaseUsersData.map((profile) => {
            ...
            return {
              ...profile,
              login:
                authUser?.raw_user_meta_data?.login ||
        ...
        console.error(
          '[API /search/combined] Critical error fetching from auth.users:',
          authFetchError
        );
      }
    }
    ...
    try {
      ...
      console.error(
        '[API /search/combined] Twitch API search error:',
        twitchError
      );
    }
    ...
    if (loginsToFetch.length > 0) {
      try {
        console.log(
          `[API /search/combined] Fetching additional user data from Twitch for: ${loginsToFetch.join(', ')}`
        );
        additionalTwitchUsers = await getTwitchUsers(loginsToFetch);
      } catch (twitchError) {
        console.error(
          '[API /search/combined] Twitch API get users error:',
          twitchError
        );
      }
    }
    ...
    twitchChannels.forEach((channel) => {
      ...
      const detailedUser = additionalTwitchUsers.find(
        (u) => u.id === channel.id
      );
      combinedResults.push({
        ...
        avatar_url:
          detailedUser?.profile_image_url || channel.thumbnail_url || null,
        registered: false,
    ...
    console.log(
      `[API /search/combined] Returning ${combinedResults.length} combined results.`
    );
    return NextResponse.json(combinedResults);
  } catch (error) {
    console.error(
      `[API /search/combined] General error for query "${query}":`,
      error
    );
    return NextResponse.json(
