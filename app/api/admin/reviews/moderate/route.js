...
async function isAdmin(token) {
  ...
  try {
    ...
    if (userError || !user) {
      console.error(
        `[isAdmin/moderate] Error verifying token:`,
        userError?.message
      );
      return false;
    }
    ...
    if (error) {
      console.error(
        `[isAdmin/moderate] Error fetching profile for ${user.id}:`,
        error.message
      );
      return false;
    }
  ...
  try {
    ...
    if (!reviewId || !status || !['approved', 'rejected'].includes(status)) {
      return NextResponse.json(
        {
          error:
            "Missing or invalid required fields (reviewId: uuid, status: 'approved' | 'rejected')",
        },
        { status: 400 }
      );
    }
    ...
    console.log(
      `[API /moderate] Admin ${user.id} attempting to set status ${status} for review ${reviewId}...`
    );
    ...
    console.log(
      `[API /moderate] Review ${reviewId} status updated to ${status} by admin ${user.id}.`
    );
    return NextResponse.json(data, { status: 200 }); // Возвращаем обновленный отзыв
  } catch (error) {
    console.error(
      `[API /moderate] Error moderating review by admin ${user.id}:`,
      error
    );
    return NextResponse.json(
