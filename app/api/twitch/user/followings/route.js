...
export async function GET() {
  try {
    ...
    if (!twitchUserId) {
      return NextResponse.json(
        { message: 'Twitch user id not found for current user' },
        { status: 400 }
      );
    }
    ...
    if (!twitchClient) {
      return NextResponse.json(
        { message: 'Failed to initialize Twitch client' },
        { status: 500 }
      );
    }

    // Получаем каналы, на которые подписан пользователь
    const follows = await twitchClient.channels.getFollowedChannels({
      userId: twitchUserId,
      limit: 100,
    });
    ...
    if (channelIds.length > 0) {
      ...
      const streams =
        await twitchClient.streams.getStreamsByUserIds(channelIds);
