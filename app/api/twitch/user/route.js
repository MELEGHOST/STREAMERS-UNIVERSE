...
import {
  getTwitchClient,
  getTwitchClientWithToken,
} from '../../../utils/twitchClient';
...
export async function GET(request) {
  ...
  if (!/^[0-9]+$/.test(userId)) {
    console.warn(
      `[API /api/twitch/user] Invalid Twitch User ID format received: ${userId}`
    );
    return NextResponse.json(
      { error: 'Invalid Twitch User ID format' },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    if (!appTwitchClient)
      throw new Error('Failed to initialize Twitch App client');

    console.log(
      `[API /api/twitch/user] Fetching public data for user ${userId} from Twitch API...`
    );
    const userResponse = await appTwitchClient.users.getUserById(userId);
    if (!userResponse) {
      console.warn(
        `[API /api/twitch/user] Twitch user with ID ${userId} not found.`
      );
      return NextResponse.json(
    ...
    console.log(
      `[API /api/twitch/user] Twitch user data fetched successfully.`
    );
    ...
    try {
      ...
      console.log(
        `[API /api/twitch/user] Fetched public followers for ${userId}: ${followersCountFromTwitch}`
      );
    } catch (publicFollowError) {
      console.error(
        `[API /api/twitch/user] Error fetching public followers for ${userId}:`,
        publicFollowError
      );
    ...
    try {
      const videosResponse = await appTwitchClient.videos.getVideosByUser(
        userId,
        { limit: 5, type: 'archive' }
      );
      ...
      console.error(
        `[API /api/twitch/user] Error fetching videos for user ${userId}:`,
        videoError
      );
    }
    ...
    console.error(
      `[API /api/twitch/user] Error fetching public Twitch data for ${userId}:`,
      error
    );
    // Если не нашли пользователя или другая ошибка Twitch API
    if (error.statusCode) {
      return NextResponse.json(
        { error: `Twitch API Error: ${error.message}` },
        { status: error.statusCode }
      );
    }
    // Если ошибка инициализации клиента или другая внутренняя
    return NextResponse.json(
      { error: error.message || 'Internal Server Error fetching Twitch data' },
      { status: 500 }
    );
  }
  ...
  try {
    if (token) {
      ...
      if (userClient) {
        const goalsRes = await userClient.callApi({
          url: 'goals',
          type: 'helix',
          query: { broadcaster_id: userId },
        });
        const items = goalsRes?.data || goalsRes?.items || [];
        const followerGoal = items.find((g) =>
          (g.type || g.goalType || '')
            .toString()
            .toLowerCase()
            .includes('follower')
        ...
        if (followerGoal) {
          followersGoalTarget =
            Number(followerGoal.target_amount ?? followerGoal.target ?? 0) ||
            null;
          followersGoalCurrent =
            Number(
              followerGoal.current_amount ??
                followerGoal.current ??
                followersCountFromTwitch ??
                0
            ) || null;
        }
      }
    ...
    console.warn(
      '[API /api/twitch/user] goals fetch skipped:',
      e?.message || e
    );
  }
  ...
  try {
    ...
    console.log(
      `[API /api/twitch/user] Looking up Supabase user ID via listUsers() for Twitch ID: ${userId}`
    );
    const {
    ...
    if (listUsersError) {
      console.error(
        `[API /api/twitch/user] Error listing users from auth.users:`,
        listUsersError
      );
    } else {
      // <<< ИЗМЕНЯЕМ ЛОГИКУ ПОИСКА >>>
      console.log(
        `[API /api/twitch/user] Searching for user with Twitch ID ${userId} in ${allAuthUsers.length} users...`
      );
      for (const u of allAuthUsers) {
        // Логируем identities, raw_user_meta_data и user_metadata
        console.log(
          `[API /api/twitch/user] Checking user ${u.id}. Identities:`,
          u.identities,
          'Raw Meta:',
        ...
        if (u.identities && Array.isArray(u.identities)) {
          const twitchIdentity = u.identities.find(
            (identity) =>
              identity.provider === 'twitch' && identity.provider_id === userId
          );
          if (twitchIdentity) {
            console.log(
              `[API /api/twitch/user] Found match in identities for user ${u.id}`
            );
            authUser = u;
            found = true;
          }
        ...
        if (!found && u.raw_user_meta_data) {
          if (u.raw_user_meta_data.provider_id === userId) {
            console.warn(
              `[API /api/twitch/user] Found user ${u.id} by raw_user_meta_data.provider_id check.`
            );
            authUser = u;
            found = true;
          } else if (u.raw_user_meta_data.user_name === userId) {
            console.warn(
              `[API /api/twitch/user] Found user ${u.id} by raw_user_meta_data.user_name check.`
            );
            authUser = u;
            found = true;
          }
        ...
        if (!found && u.user_metadata) {
          if (u.user_metadata.provider_id === userId) {
            console.warn(
              `[API /api/twitch/user] Found user ${u.id} by user_metadata.provider_id check.`
            );
            authUser = u;
            found = true;
          } else if (u.user_metadata.user_name === userId) {
            console.warn(
              `[API /api/twitch/user] Found user ${u.id} by user_metadata.user_name check.`
            );
            authUser = u;
            found = true;
          }
        ...
        if (found) {
          ...
          console.log(
            `[API /api/twitch/user] No match for user ${u.id} in any checked fields.`
          );
        }
      }
      ...
      if (!authUser) {
        console.log(
          `[API /api/twitch/user] User with Twitch ID ${userId} not found after checking all fields.`
        );
      }
    }
    ...
    if (authUser) {
      ...
      console.log(
        `[API /api/twitch/user] Found Supabase User ID: ${supabaseUserIdFromTwitchId} for Twitch ID ${userId} (via identities)`
      );
      ...
      const { data: profile, error: profileError } = await supabaseAdmin
        .from('user_profiles')
        .select(
          'description, role, social_links, profile_widget, twitch_broadcaster_type'
        )
      ...
      if (profileError) {
        console.error(
          `[API /user] Ошибка получения профиля из БД для ${userId}:`,
          profileError
        );
      } else if (profile) {
        profileData = profile;
        console.log(
          `[API /api/twitch/user] User profile found in Supabase for user_id ${supabaseUserIdFromTwitchId}`
        );
      } else {
        console.log(
          `[API /api/twitch/user] User profile NOT found in Supabase for user_id ${supabaseUserIdFromTwitchId}`
        );
      }
    } else {
      console.log(
        `[API /api/twitch/user] Could not find Supabase User ID for Twitch ID ${userId}. User might not be registered.`
      );
    }
    ...
    if (token) {
      ...
      if (
        verifiedToken &&
        supabaseUserIdFromTwitchId &&
        verifiedToken.sub === supabaseUserIdFromTwitchId
      ) {
        isOwnerViewing = true;
        console.log(
          `[API /api/twitch/user] Logged-in user ${verifiedToken.sub} owns the requested profile ${userId}.`
        );
      } else if (verifiedToken) {
        console.log(
          `[API /api/twitch/user] Logged-in user ${verifiedToken.sub} does NOT own the requested profile ${userId} (Owner Supabase ID: ${supabaseUserIdFromTwitchId}).`
        );
      } else {
        console.warn(
          `[API /api/twitch/user] Invalid or expired token provided.`
        );
      }
    } else {
      console.log(
        `[API /api/twitch/user] No token provided, cannot determine owner.`
      );
    }
  } catch (lookupError) {
    console.error(
      '[API /api/twitch/user] Error during profile lookup:',
      lookupError
    );
  }
  ...
  console.log(
    `[API /api/twitch/user] Checking ownership before update: isOwnerViewing = ${isOwnerViewing}`
  );
  if (isOwnerViewing && supabaseUserIdFromTwitchId && twitchUserData) {
    try {
      ...
      console.log(
        `[API /api/twitch/user] Using public follower count for owner update: ${currentFollowersCount}`
      );

      const currentBroadcasterType = twitchUserData.broadcaster_type;
      const currentRole = determineRole(
        currentFollowersCount,
        currentBroadcasterType
      );
      console.log(
        `[API /api/twitch/user] Determined role for owner ${userId}: ${currentRole}`
      );
      ...
      console.log(
        `[API /api/twitch/user] Updating profile data for owner ${currentSupabaseUserId}. Data:`,
        updatePayload
      );
      ...
      if (updateErrorDb) {
        console.error(
          `[API /api/twitch/user] Error updating profile data for ${currentSupabaseUserId}:`,
          updateErrorDb
        );
      } else if (updatedProfile) {
        console.log(
          `[API /api/twitch/user] Profile data updated successfully for ${currentSupabaseUserId}.`
        );
        profileData = profileData || updatedProfile;
      } else {
        console.log(
          `[API /api/twitch/user] Profile not found for ${currentSupabaseUserId}. Skipping create to avoid DB trigger errors.`
        );
      }
    } catch (updateError) {
      console.error(
        `[API /api/twitch/user] Error during owner profile data update process:`,
        updateError
      );
    }
  } else {
    console.log(
      `[API /api/twitch/user] Skipping owner profile update. isOwnerViewing: ${isOwnerViewing}, supabaseUserId: ${supabaseUserIdFromTwitchId}, twitchUserData: ${!!twitchUserData}`
    );
  }

  // --- Финальная подготовка данных для ответа ---
  if (twitchUserData) {
    twitchUserData.followers_count =
      followersCountFromTwitch ?? twitchUserData.followers_count;
  ...
  console.log(
    `[API /api/twitch/user] Returning final profileData:`,
    profileData
  );
