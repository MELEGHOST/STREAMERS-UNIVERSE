  ...
  try {
    ...
    try {
      ...
      if (tokenError.response && tokenError.response.status === 401) {
        ...
        // Не продолжаем выполнение, если проверка не удалась и это не ошибка 401
        console.log('Profile API - Ошибка проверки токена, не продолжаем выполнение');
        return NextResponse.json({ 
          error: 'Ошибка проверки токена', 
          message: 'Произошла ошибка при проверке токена доступа. Пожалуйста, войдите снова.' 
        }, { status: 401 });
      }
    }
