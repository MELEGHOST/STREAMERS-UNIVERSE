...
async function getYoutubeAudioStream(url) {
  ...
  try {
    ...
    if (!format) {
      throw new Error(
        'Не найден подходящий аудиоформат для этого YouTube видео.'
      );
    }
    console.log(
      `[getYoutubeAudioStream] Chosen format: ${format.mimeType}, ${format.audioBitrate}kbps`
    );
    ...
    if (originalErrorMessage.includes('Status code: 410')) {
      userFriendlyMessage =
        'Не удалось получить аудио с YouTube (Ошибка 410). Возможно, видео недоступно в регионе сервера (США), удалено или имеет возрастные/региональные ограничения.';
    }
...
export async function POST(request) {
  ...
  if (!supabaseUrl || !supabaseServiceKey) {
    console.error(
      '[API /generate-full] Critical Error: Supabase keys missing!'
    );
  ...
  if (!openRouterApiKey) {
    console.error(
      '[API /generate-full] Critical Error: OPENROUTER_API_KEY missing!'
    );
    return NextResponse.json(
  ...
  try {
    ...
    if (!authorTwitchName || (!sourceFilePath && !sourceUrl)) {
      console.error(
        '[API /generate-full] Validation failed: Missing required fields',
        { authorTwitchName, sourceFilePath, sourceUrl }
      );
      return NextResponse.json(
        {
          error:
            'Missing required fields (authorTwitchName, and either sourceFilePath or sourceUrl)',
        },
        { status: 400 }
      );
    }
    if (sourceFilePath && sourceUrl) {
      console.error(
        '[API /generate-full] Validation failed: Both sourceFilePath and sourceUrl provided'
      );
      return NextResponse.json(
        { error: 'Provide either sourceFilePath or sourceUrl, not both.' },
        { status: 400 }
      );
    }
    // --------------------------
    ...
    console.log(
      `[API /generate-full] User ${userId} processing request for AI review. Author: ${authorTwitchName}`
    );
    ...
    if (sourceFilePath) {
      ...
      console.log(
        `[API /generate-full] Processing uploaded file: ${sourceFilePath}`
      );
      ...
      console.log(
        `[API /generate-full] Downloaded file type: ${fileType}, name: ${sourceFileName}`
      );
      ...
      try {
        ...
        // }
          ...
          throw new Error(
            'Неподдерживаемый URL. Сейчас поддерживаются только ссылки YouTube.'
          );
        }
      } catch (urlProcessingError) {
        console.error(
          `[API /generate-full] Error processing URL ${sourceUrl}:`,
          urlProcessingError
        );
        return NextResponse.json(
          {
            error:
              urlProcessingError.message || 'Ошибка обработки URL источника.',
          },
          { status: 400 }
        );
      }
    ...
    if (streamSource) {
      console.log(
        `[API /generate-full] Sending stream/file '${sourceFileName}' to Whisper...`
      );
      try {
        const transcriptionResponse =
          await openrouter.audio.transcriptions.create({
        ...
        if (typeof transcriptionResponse === 'string') {
          ...
          typeof transcriptionResponse.text === 'string'
        ) {
          fileContentText = transcriptionResponse.text.trim();
        } else {
          console.error(
            '[API /generate-full] Whisper did not return valid text.',
            transcriptionResponse
          );
          throw new Error('Ошибка транскрипции: Whisper не вернул текст.');
        }
        console.log(
          `[API /generate-full] Whisper transcription successful (${fileContentText.length} chars).`
        );
      } catch (transcriptionError) {
        console.error(
          '[API /generate-full] Whisper transcription failed:',
          transcriptionError
        );
        return NextResponse.json(
          {
            error: `Ошибка транскрипции из ${processingSource}: ${transcriptionError.message}`,
          },
          { status: 500 }
        );
      }
      ...
      console.error(
        '[API /generate-full] No stream source and no text content found.'
      );
      return NextResponse.json(
        {
          error:
            'Не удалось получить контент для генерации (ни текст, ни аудио/видео).',
        },
        { status: 400 }
      );
    }
    ...
    if (!fileContentText) {
      console.error(
        '[API /generate-full] Text content is empty after processing source.'
      );
      return NextResponse.json(
        {
          error:
            'Не удалось получить текстовое содержимое для генерации отзыва.',
        },
        { status: 500 }
      );
    }
    ...
    const truncatedContent =
      fileContentText.length > maxContentLength
        ? fileContentText.substring(0, maxContentLength) +
          '... [содержимое обрезано]'
        : fileContentText;
    ...
    try {
      ...
      console.log(
        `[API /generate-full] Ищем автора в БД по нику: ${nicknameLower}`
      );
      ...
      const { data: profileData, error: profileError } = await supabaseAdmin
        ...
        .or(
          `user_metadata->>user_name.eq.${nicknameLower},raw_user_meta_data->>login.eq.${nicknameLower},raw_user_meta_data->>name.eq.${nicknameLower}`
        )
        .maybeSingle();

      if (profileError)
        throw new Error(
          `Database error (author lookup): ${profileError.message}`
        );
      ...
      if (profileData) {
        ...
        console.log(
          `[API /generate-full] Автор ${nicknameLower} найден в БД. User ID: ${authorUserId}`
        );
      } else {
        console.log(
          `[API /generate-full] Автор ${nicknameLower} не найден в БД. Проверяем на Twitch...`
        );
        validatedTwitchUser = await validateTwitchUser(nicknameLower);
        if (!validatedTwitchUser) {
          console.warn(
            `[API /generate-full] Автор с ником ${authorTwitchName} не найден и на Twitch.`
          );
          return NextResponse.json(
            {
              error: `Пользователь Twitch с никнеймом '${authorTwitchName}' не найден.`,
            },
            { status: 404 }
          );
        } else {
          console.log(
            `[API /generate-full] Автор ${authorTwitchName} найден на Twitch. ID: ${validatedTwitchUser.id}, Display Name: ${validatedTwitchUser.display_name}`
          );
          streamerTwitchId = validatedTwitchUser.id;
          streamerDisplayName =
            validatedTwitchUser.display_name ||
            validatedTwitchUser.displayName ||
            null;
      ...
      console.error(
        `[API /generate-full] Ошибка поиска/валидации автора ${authorTwitchName}:`,
        dbLookupError
      );
      let statusCode = 500;
      let message = `Ошибка поиска автора: ${dbLookupError.message || dbLookupError}`;
      if (dbLookupError.message?.includes('Twitch API request failed'))
        statusCode = 503;
    ...
    console.log(
      `[API /generate-full] Generating review. Author: ${authorTwitchName} (User ID: ${authorUserId ?? 'N/A'}), Source: ${processingSource}`
    );
...
Please generate the review JSON based on this content. Remember the JSON format with keys: review_text, rating, category, item_name, subcategory (optional).`;
    ...
    console.log(
      `[API /generate-full] Sending request to AI. Prompt size approx: ${systemPrompt.length + userPrompt.length}`
    );
    ...
    console.log(
      '[API /generate-full] Raw AI Response:',
    ...
    if (!aiResultString) {
      console.error(
        `[API /generate-full] AI returned empty or no content. Finish reason: ${choiceReason}.`
      );
      throw new Error(
        `AI не вернул контент ответа. Причина завершения: ${choiceReason || 'unknown'}`
      );
    }
    console.log(
      `[API /generate-full] AI response string received (${aiResultString.length} chars). Finish reason: ${choiceReason}.`
    );
    ...
    try {
      ...
      console.error(
        '[API /generate-full] Failed to parse AI JSON response:',
        aiResultString
      );
    ...
    if (
      ...
      console.error(
        '[API /generate-full] Invalid JSON structure from AI:',
        reviewJson
      );
    ...
    if (insertError) {
      console.error(
        `[API /generate-full] DB insert error for review by ${userId} about ${authorTwitchName}:`,
        insertError
      );
    ...
    console.log(
      `[API /generate-full] Generated review saved with ID: ${reviewData?.id} by user ${userId} about author: ${authorTwitchName}`
    );
    return NextResponse.json(
      {
        message: 'Review generated and submitted for moderation.',
    ...
    console.error(
      '[API /generate-full] Unexpected error in POST handler:',
      error
    );
