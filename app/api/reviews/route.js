...
export async function GET() {
  ...
  if (!supabaseUrl || !supabaseServiceKey) {
    console.error(
      '[API /reviews] Critical Error: Supabase URL or Service Key is missing!'
    );
  ...
  try {
    ...
    console.log(
      `[API /api/reviews] Found ${data?.length ?? 0} approved reviews.`
    );
...
export async function POST(request) {
  ...
  try {
    ...
    // Простая валидация
    if (!category || !item_name || !rating || !text) {
      // Проверяем text
      return NextResponse.json(
    ...
    if (
      (category === 'Фильмы' || category === 'Сериалы') &&
      (!genres || !Array.isArray(genres) || genres.length === 0)
    ) {
      return NextResponse.json(
        { error: 'Genres are required for Movies/Series' },
        { status: 400 }
      );
    }
    ...
    if (!(category === 'Фильмы' || category === 'Сериалы') && !subcategory) {
      return NextResponse.json(
        { error: 'Subcategory is required for this category' },
        { status: 400 }
      );
    }

    console.log(
      `[API /api/reviews] Creating manual review by ${userId} for item: ${item_name}`
    );

    // Отзывы всегда одобрены
    const reviewStatus = 'approved';
    console.log(
      `[API /api/reviews] Manual review status set to: ${reviewStatus}`
    );
    ...
    const { data: reviewData, error: insertError } = await supabaseAdmin
      ...
      .insert({
        ...
        subcategory:
          category !== 'Фильмы' && category !== 'Сериалы' ? subcategory : null, // Сохраняем подкатегорию
      })
      .select('id, status')
      .single();
    ...
    if (insertError) {
      ...
      if (
        insertError.message.includes('rating') &&
        insertError.message.includes('does not exist')
      ) {
        return NextResponse.json(
          { error: 'Database error: rating column missing.' },
          { status: 500 }
        );
      }
      return NextResponse.json(
    ...
    console.log(
      `[API /api/reviews] Manual review created by ${userId} with ID: ${reviewData.id}, Status: ${reviewData.status}`
    );
    ...
    return NextResponse.json(
      { id: reviewData.id, status: reviewData.status, message },
      { status: 201 }
    );
  } catch (error) {
