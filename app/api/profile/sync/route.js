...
export async function POST({ headers }) {
  ...
  if (userError || !user) {
    console.error(
      '[Sync Profile API] Ошибка получения пользователя по JWT:',
      userError?.message
    );
    return NextResponse.json(
  ...
  try {
    ...
    if (providerToken) {
      ...
      if (updateUserError) {
        console.error(
          `[Sync Profile API] Ошибка обновления user_metadata для ${user.id}:`,
          updateUserError.message
        );
        // Не фатальная ошибка, продолжаем выполнение
      } else {
        console.log(
          `[Sync Profile API] Успешно сохранен provider_token для ${user.id}`
        );
      }
    } else {
      console.warn(
        `[Sync Profile API] provider_token не найден в JWT для пользователя ${user.id}. Пропускаем обновление.`
      );
    }
  } catch (e) {
    console.error(
      `[Sync Profile API] Не удалось декодировать JWT для извлечения provider_token:`,
      e.message
    );
  }
  ...
  if (profileError && profileError.code !== 'PGRST116') {
    console.error(
      '[Sync Profile API] Ошибка при поиске профиля:',
      profileError
    );
    return NextResponse.json(
      { error: 'Ошибка базы данных при поиске профиля.' },
      { status: 500 }
    );
  }
  ...
  const twitchUser =
    twitchClient && user.user_metadata.provider_id
  ...
  if (profile) {
    ...
    if (profile.twitch_broadcaster_type !== broadcasterType) {
      await supabaseAdmin
        .from('user_profiles')
        .update({ twitch_broadcaster_type: broadcasterType })
        .eq('user_id', user.id);
      console.log(
        `[Sync Profile API] Updated broadcaster_type for user ${user.id} to '${broadcasterType}'`
      );
    }
  ...
  if (createError) {
    console.error(
      '[Sync Profile API] Ошибка при создании профиля:',
      createError
    );
    return NextResponse.json(
