...
export default function EditReviewPage() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 404) {
          ...
          throw new Error(
            errorData.error ||
              t('edit_review.serverError', { status: response.status })
          );
        }
      }
      ...
      console.error(
        `[EditReviewPage] Error fetching review ${reviewId}:`,
        fetchError
      );
  ...
  useEffect(() => {
    if (isAuthenticated !== null) {
      // Ждем определения статуса аутентификации
      if (isAuthenticated) {
        fetchReviewData();
      } else {
        setError(t('edit_review.authRequired'));
  ...
  const handleSave = async (event) => {
    ...
    try {
      ...
      console.log(
        `[EditReviewPage] Review ${reviewId} saved successfully. Response:`,
        result
      );
      ...
      console.error(
        `[EditReviewPage] Error saving review ${reviewId}:`,
        saveError
      );
  ...
  if (error && !reviewData) {
    // Показываем критическую ошибку, если данные не загрузились
    return (
  ...
  return (
    <div className={pageStyles.container}>
      ...
      <p className={styles.itemName}>
        {t('edit_review.objectLabel', {
          name: reviewData.item_name || t('edit_review.unknown'),
        })}
      </p>
      <p className={styles.category}>
        {t('edit_review.categoryLabel', {
          category: reviewData.category || t('edit_review.unknown'),
        })}
      </p>
      {error && <p className={pageStyles.errorMessage}>{error}</p>}{' '}
      {/* Ошибки валидации/сохранения */}
      <form onSubmit={handleSave} className={styles.editForm}>
        <div className={styles.formGroup}>
          <label htmlFor="rating" className={styles.label}>
            {t('edit_review.ratingLabel')}
          </label>
          <StarRating
            rating={editRating}
        ...
        <div className={styles.formActions}>
          <button
            ...
            onClick={() => router.push('/my-reviews')}
            disabled={isSaving}
            className={`${pageStyles.button} ${styles.cancelButton}`}
          >
          ...
          <button
            type="submit"
            disabled={isSaving}
            className={`${pageStyles.button} ${styles.saveButton}`}
          >
            {isSaving
              ? t('edit_review.savingButton')
              : t('edit_review.saveButton')}
          </button>
        </div>
      </form>
    </div>
