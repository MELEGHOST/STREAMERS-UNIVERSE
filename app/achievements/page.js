...
function AchievementCard({ achievement, t }) {
  ...
  return (
    <div
      className={`${styles.achievementCard} ${achievement.is_unlocked ? styles.unlocked : styles.locked}`}
    >
...
function AchievementsPageContent() {
  ...
  const {
    ...
    ([url, token]) => fetcher(url, token),
    {
      ...
      onError: (err) => {
        console.error('[AchievementsPage useSWR onError]', err);
      },
      errorRetryCount: 3,
  ...
  const error = apiError
    ? apiError.message === 'No response from server'
      ? 'Сервер не отвечает, попробуй позже, бро!'
      : apiError.message
    : null;
  ...
  return (
    <div className={pageStyles.container}>
      ...
      <div className={styles.tabs}>
        ...
        >
          {t('profile_page.achievements_page.unlocked')} (
      ...
      {error && (
        <div
          ...
          style={{ marginBottom: '1rem' }}
        >
          {t('achievements.page.error', {
            defaultValue: 'Error loading achievements',
          })}{' '}
      ...
      <div className={styles.achievementsList}>
        ...
        {activeTab === 'all' &&
          (achievements.length === 0 ? (
            <p className={styles.noAchievements}>
              {t('profile_page.achievements_page.noAchievementsAll', {
