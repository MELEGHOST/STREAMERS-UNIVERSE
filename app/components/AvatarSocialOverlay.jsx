...
const platformMeta = {
  twitch: { label: 'TW', color: '#9146ff', base: (v) => `https://twitch.tv/${v}` },
  youtube: { label: 'YT', color: '#ff0000', base: (v) => `https://youtube.com/${v}` },
  ...
  tiktok: { label: 'TT', color: '#000000', base: (v) => `https://www.tiktok.com/@${v}` },
  yandex_music: { label: 'YAM', color: '#ffcc00', base: () => null },
  boosty: { label: 'BO', color: '#ff6a00', base: (v) => `https://boosty.to/${v}` },
  instagram: { label: 'inst', color: '#E4405F', base: (v) => `https://instagram.com/${v}` },
  x: { label: 'X', color: '#000000', base: (v) => `https://x.com/${v}` },
  kick: { label: 'kick', color: '#53FC18', base: (v) => `https://kick.com/${v}` },
  facebook: { label: 'face', color: '#1877f2', base: (v) => `https://facebook.com/${v}` },
  reddit: { label: 'radd', color: '#FF4500', base: (v) => `https://reddit.com/u/${v}` },
  steam: { label: 'STEM', color: '#171a21', base: (v) => `https://steamcommunity.com/id/${v}` },
};
...
export default function AvatarSocialOverlay({
  ...
  const entries = SUPPORTED_PLATFORMS
      ...
      <div className={styles.center}>
        ...
        <div className={styles.ring} style={{ ['--count']: entries.length }}>
          {entries.map(([platform, value], index) => {
            const meta = platformMeta[platform] || { label: platform, color: '#888' };
