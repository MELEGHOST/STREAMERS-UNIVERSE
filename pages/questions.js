...
export default function Questions() {
      ...
      try {
        ...
        if (storedMessages) {
          try {
            ...
            // Фильтруем сообщения пользователя (только вопросы)
            userMessages = parsedMessages.filter(msg => 
              (msg.senderId === userId || msg.receiverId === userId) && 
              (msg.type === 'question' || !msg.type) // Учитываем старые сообщения без типа
            );
            ...
            setError('Ошибка при загрузке вопросов. Пожалуйста, попробуйте позже.');
          }
        }
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      const newQuestionObj = {
        ...
        read: false,
        type: 'question' // Добавляем тип "вопрос"
      };
  ...
  if (!isAuthenticated) {
    return (
      <div className={styles.container}>
        <Head>
          <title>Вопросы | Streamers Universe</title>
        </Head>
        <div className={styles.authMessage}>
          <h2>Требуется авторизация</h2>
          <p>Пожалуйста, войдите в систему, чтобы просматривать вопросы.</p>
  ...
  return (
    <div className={styles.container}>
      <Head>
        <title>Вопросы | Streamers Universe</title>
        <meta name="description" content="Вопросы в Streamers Universe" />
      </Head>
        ...
        <div className={styles.conversationsList}>
          ...
          {loading ? (
            ...
            <div className={styles.emptyState}>
              ...
              <p className={styles.emptyStateSubtext}>Перейдите в профиль пользователя, чтобы задать вопрос</p>
            </div>
          ) : (
        ...
        <div className={styles.messagesArea}>
          {activeConversation ? (
              ...
              <div className={styles.messagesContent}>
                {messages.length === 0 ? (
                  <div className={styles.emptyMessages}>
                    <p>Начните диалог с {getConversationName(activeConversation)}</p>
                  </div>
                ) : (
            ...
            <div className={styles.noActiveConversation}>
              ...
              <p>Выберите диалог из списка слева или начните новый, перейдя в профиль пользователя</p>
            </div>
          )}
        </div>
      </div>
